...
bool Hedcut::build(cv::Mat & input_image, int n)
{
	...
	cvt.channel_image = this -> channel_image;
	...
	else if(cvt.channel_image)
	{
		cv::Mat bgr[3];
		cv::split(input_image, bgr);
		...
		for(int i=0; i<3; ++i)
		{
			...
			params.channel_image = bgr[i];
			params.color_id = i;
			threads.push_back(std::thread(&Hedcut::compute_channel_image, this, params));
		}
  ...
  //       //sample n points
	 //    sample_initial_points(bgr[2], n, pts);
		// cvt.compute_weighted_cvt(bgr[2], pts);
		// create_disks(input_image, cvt);
 	// 	std::cout << "Finished red channel" << "\n";
 
       
		//  //sample n points
	 //    sample_initial_points(bgr[0], n, pts);
		// cvt.compute_weighted_cvt(bgr[0], pts);
		// create_disks(input_image, cvt);
  //       std::cout << "Finished blue channel" << "\n";

 	//     //sample n points
	 //    sample_initial_points(bgr[1], n, pts);
		// cvt.compute_weighted_cvt(bgr[1], pts);
		// create_disks(input_image, cvt);
		// std::cout << "Finished green channel" << "\n";
...
void Hedcut::create_disks(cv::Mat & img, CVT & cvt)
{
	...
	for (auto& cell : cvt.getCells())
	{
		...
		disk.radius = (uniform_disk_size) ? disk_size :  2*(100 * disk_size / (avg_v + 100));
...
void Hedcut::compute_channel_image(ChannelParams params)
{
	int n = params.n;
	// cv::Mat img = params.channel_image.clone();
	cv::Mat input_image = params.input_image.clone();
	cv::Mat channelimg = params.channel_image.clone();

	CVT converter;
	converter.iteration_limit = this->cvt_iteration_limit;
	converter.max_site_displacement = this->max_site_displacement;
	converter.average_termination = this->average_termination;
	converter.gpu = this->gpu;
	converter.subpixels = this->subpixels;
	converter.debug = this->debug;
	converter.channel_image = this -> channel_image;
	converter.id = params.color_id;

	//sample n points
	std::vector<cv::Point2d> pts;
	 //sample n points
    sample_initial_points(channelimg, n, pts);
 
	converter.compute_weighted_cvt(channelimg, pts);
	
	create_disks(input_image, converter);
}